...
Get information on Ecoregions and Biomes by spatial intersection. 
```{r get-ecoregions}
eco_data_path <- stringr::str_glue("{derived_data_path}/ecoregions2017.gpkg")
if( !file.exists(eco_data_path) ){
  
  download.file(url = "https://storage.googleapis.com/teow2016/Ecoregions2017.zip", 
                destfile = stringr::str_glue("{raw_data_path}/ecoregions2017.zip"))  
  
  unzip(zipfile = stringr::str_glue("{raw_data_path}/ecoregions2017.zip"), 
        overwrite = TRUE, exdir = raw_data_path)
  
  ecoregions <- sf::st_read(dsn = stringr::str_glue("{raw_data_path}/Ecoregions2017.shp"), 
              stringsAsFactors = FALSE) %>% 
    dplyr::rename(ID_ECO = ECO_ID, geom = geometry) 
  
  aux <- suppressWarnings(split(ecoregions, ecoregions$ID_ECO)) %>%
    parallel::mclapply(mc.cores = parallel::detectCores(), 
                       FUN = function(x) sf::st_make_valid(x) %>% sf::st_simplify()) %>% 
    do.call("rbind", .) %>% 
    sf::st_write(layer = "ecoregions", dsn = eco_data_path, delete_dsn = TRUE)
  
}

ecoregions <- sf::st_read(dsn = eco_data_path, 
                          query = "SELECT ID_ECO, geom FROM ecoregions") %>% 
  sf::st_cast("POLYGON") # Casting to polygons makes the intersects much faster

system.time(
  mining_env_extent <- sf::st_join( mining_pol,
                                    y = ecoregions,
...
mining_env_extent %>% 
  sf::st_drop_geometry() %>% 
  tibble::as_tibble() %>% 
  dplyr::select(FID_POL = FID, ID_ECO) %>% 
  readr::write_csv(stringr::str_glue("{derived_data_path}/ecoregion_ids.csv"))
```
